...
// GetTopMovies 影片Top排行：以播放记录为准，按 data_id 分组统计。
// 每条播放记录记一次（play_count）。不再提前按“时长”取前N——把所有分组行返回，
// 由前端在完整集合上按 时长/次数 独立排序截取，避免两个 Tab 共用同一批“时长Top”影片。
func (r *RepositoryPlayHistoryCRUD) GetTopMovies(userId string, galleryUid string, startDate string, endDate string, limit int) ([]repository.MoviePlayStat, error) {
	...
	go func(ch chan<- bool) {
		...
		// 不下 Order/Limit：返回全部分组行，排序截断交给前端，保证“次数 Top”能看到真正的次数排行
